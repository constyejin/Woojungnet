...
// 입력된 데이타의 한글 비정상여부의 기존 펑션에 체크 콤마(,)를 추가한다.
function isValidAddComma(varCk, charSet) {
        var chk=true;
        for (i=0; i<varCk.length; i++) {
                ch = varCk.charAt(i);
                if ((ch >= "가" && ch <= "\uD7A3") || (ch>="0" && ch<="9")
                    || (ch>="a" && ch<="z") || (ch>="A" && ch<="Z") || (ch==",")) {
                        ...
                        if (!chk)       break;  // 한글+특수문자외의 문자가 있는 경우만 error 종료 2004-10-20 3:28오후
                }
        }
        return chk;
}
// 2004-10-20 3:28오후 윤성희 추가
...
// 2005-07-27 10:59오전  tableOver tableOut
// visibility 속성과 display 속성은 따로 처리된다.
//테이블 레이어
...
function checkReqer(chkIAType){
    ...
    //요청번호:[3906]상품 신청화면 권한 부여-유통망과 CM 간의 커뮤니케이션 촉진-조회만 가능 토록:박경미:2007-09-19
	/* 잠시 막음
    if( form.USR_GB.value == "5") {
    	alert("오더저장 할 수 있는 권한이 없습니다.");
    	return false;
    }
    */
    //var varSpecialChar = '[]~!@#$%^&*()_+|`-=\[]{}:\"<>?,./ 0123456789';
    ////////////
    // 2005-03-07 3:41오후 * 제거
    var varSpecialChar = '[]~!@#$%^&()_+|`-=\[]{}:\"<>?,./ 0123456789';
    ...
    if (isLength(form.REQER_NAME.value) > 50) {
        alert("신청자명을 올바르게 입력해 주십시오!(최대 50자리까지 입력가능)");
        form.REQER_NAME.focus();
        return false;
    }
    ...
    if ( !eval(form.REQER_CNTC_NO_JJ) ) {
        ...
        if( isLength(tmpREQER_CNTC_NO) == 0 && isLength(tempREQER_CNTC_HP_NO)==0) {
        	alert("신청자 정보의 휴대전화나 자택전화 중 하나는 반드시 입력해야 합니다.");
        	return false;
        }
        //2007-08-08 END
        ...
        if( isLength(form.REQER_CNTC_NO1.value) > 0){
            ...
            if( !isCellarPhone(form.REQER_CNTC_NO1.value)){
                alert('신청자 사항의 휴대전화의 지역번호를 확인하세요.');
            ...
            if( isLength(form.REQER_CNTC_NO3.value) != 4){
                alert('신청자 사항의 휴대전화의 전화번호를 확인하세요.');
        ...
        */
        ...
        if( isLength(form.REQER_CNTC_HP_NO1.value) > 0){
            ...
            if( !isGenPhone(form.REQER_CNTC_HP_NO1.value) && !isVoipCheck(form.REQER_CNTC_HP_NO1.value) && !isPnaPhone(form.REQER_CNTC_HP_NO1.value)){
                alert('신청자 사항의 자택전화의 지역번호를 확인하세요.');
            ...
            if( isLength(form.REQER_CNTC_HP_NO3.value) != 4){
                alert('신청자 사항의 자택전화의 전화번호를 확인하세요.');
...
// 수정자   : 이은미
// 수정일   : 2007-06-01
// 수정내용 : 일반전화일경우만 csng101s 서비스 조회를 위해 - 주소조회시 pagecd추가
//--------------------------------------------------------------------------------
...
function paComm_Popup_Address(ObjAddr, vModal)
{
    ...
    try
    {
        ...
        // 보조주소도 보여줌 2006-08-12 5:35오후오동원
        try{
            params = params + "&bldg_ref="      + ObjAddr.bldg_ref;                     // 보조주소도 보여줌 2006-08-12 5:35오후오동원
            params = params + "&bldgtypecd="    + ObjAddr.bldgtypecd;                   // 보조주소도 보여줌 2006-08-12 5:35오후오동원
            /////////////
            // 아파트일경우도.. 처리함 동 호수처리
            params = params + "&addr_bldg_ho="      + ObjAddr.addr_bldg_ho;     // 보조주소도 보여줌 2006-08-12 5:35오후오동원
            params = params + "&addr_bldg_dong="    + ObjAddr.addr_bldg_dong;   // 보조주소도 보여줌 2006-08-12 5:35오후오동원
        }catch(e){
        ...
        // BCN - CSNG101S서비스 사용 일반전화일때만
        // 2007-06-01 5:16오후
        try {
        	if(ObjAddr.pagecd != null && ObjAddr.pagecd != "undefined") {
            	params = params + "&pagecd="    + ObjAddr.pagecd;       // 일반전화일때만
            } else {
            	params = params + "&pagecd="    + "";
            }
        } catch (e) {
            params = params + "&pagecd="+"";       // 사용자구분
        }
...
// 수정자   : 이은미
// 수정일   : 2007-03-08
// 수정내용 : addr_bldg_no 인자값에 특수문자(#)가 들어가 치환
//---------------------------------------------------------------------------------------
function paComm_Popup_CompleteDate(rObj, flag)
{
    ...
    try{
        new_mega = rObj.new_mega;                    // NEW_MEGA -> 계약자체크를 생략한다. 2006-06-08 9:30오전
    } catch(e) {};
    ...
    // 2006-05-16 5:45오후 이재용
    // NEW_MEGA는 계약자 사항 조회 체크를 생략한다.
    if(new_mega != 'Y')
    {
    ...
    // 2005-03-09 개통희망일 보여주기
    // Nespot ID 가 사용중이거나 청구중이면 토,일요일 신청 불가처리
    var vNespotIdFlag   = "N";
...
return;
    ...
    //-----------------------------------------------------------------------------
    // 요청번호 : PENALTY_FLAG 체크...
    // 요청자   : 위주양
    // 수정자   : 이재용
    // 수정일   : 2006.03.06
    // 수정내용 :  PENALTY_FLAG 체크...
    //-----------------------------------------------------------------------------
    var vPENALTY_FLAG    = '';
    var vZZ_NO_MENU_FLAG = '';

    //-----------------------------------------------------------------------------
    // 요청번호 : [3443] 고객정보 수집하는 화면에 ssl 적용
    // 요청자   : 김미경
    // 수정자   : 이재용
    // 수정일   : 2007-05-11 10:01오전
    // 수정내용 : SSL 변수 로딩
    //-----------------------------------------------------------------------------
    var vSSL_HTTPS = '';
    ...
    try
    {
        ...
        {
            var vPenaltyMsg = '영업정지된 유통망입니다. 영업정지 기간중에는 상품신청을 하실수 없습니다.';
...
//-----------------------------------------------------------------------------
// 요청번호 : [2936] 비업무시간 상담 예약 포워딩, 이광석 20070406
// 요청자   : 김미경
// 수정자   : 이광석
// 수정일   : 2007-04-06 1:45오후
// 수정내용 : 설변장치비 실비항목설정
//-----------------------------------------------------------------------------
                var wd;
                if(form.USR_GB.value == "1" && confirm("상담 예약을 신청하시겠습니까?")) {
    ...
    //////////////////
    // 2005-11-24 4:56오후 oracle 9i up
...
//-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    // 요청번호 : [3431] 상품신청 구화면에 공지사항 출력
    // 요청자   : 고겸
    // 수정자   : 이재용
    // 수정일   : 2007-05-07 2:49오후
    // 수정내용 : 구메뉴 Close 안내
    //-----------------------------------------------------------------------------
    try
    {
        ...
        if( form.USR_GB.value == "5" || form.USR_GB.value == "6" )
        {
            if( vA4SMenuId == 'A143' || // A143	일반전화 신청(구화면)
                vA4SMenuId == 'A144' || // A144	설치장소변경(구화면)
                vA4SMenuId == 'A145' || // A145	번호변경(구화면)
                vA4SMenuId == 'A146' || // A146	LM더블프리 신청(구화면)
                vA4SMenuId == 'A147' || // A147	마이스타일요금제 (구화면)
                vA4SMenuId == 'A148' || // A148	링고 신청(구화면)
                vA4SMenuId == 'A202' || // A202	Megapass Lite(구)
                vA4SMenuId == 'A203' || // A203	Megapass Premium 신청(구)
                vA4SMenuId == 'A204' || // A204	Megapass Special 신청(구)
                vA4SMenuId == 'A205' || // A205	Megapass Ntopia 신청(구)
                vA4SMenuId == 'A219' || // A219	Nespot Solo Lite(구)
                vA4SMenuId == 'A220' || // A220	Nespot Solo Premium(구)
                vA4SMenuId == 'A221' || // A221	Nespot Solo Ntopia(구)
                vA4SMenuId == 'A222' )  // A222	Nespot Solo Special(구)
            {
...
//-----------------------------------------------------------------------------
    ...
    function custInfoUse(cust_id,cust_no_type,rs_cd ,methodname){
        ...
        // 2005-08-30 5:37오후 개인별 고객동의사항 사용안함
        try{
            //callOrdSaveProc();
            eval( methodname + "();");
        }catch(e){
...
function chkCcui122s(arrCcui) {
	...
	// arrCcui[0] 은 사이버 일경우 어디로 튕길지의 여부
	// mall : 상품몰쪽으로 change : 상품변경쪽으로
...
///////////////////////////
//// [4738] 사내직원이 고객의 Self신청 조력에 대한 판매자(권유자) 정보 입력기능 개발) 2008-03-10 오후 8:22:56 박민진
...
/******************************************************************************
* 전화관련 DB 작업으로 인한 작업 제한 공지 2006-09-21 7:08오후
******************************************************************************/
...
//////////////////////////////////////////////
// 브렌드 사이트에 SO iframe방식으로 제공
var MSG_PAGELOADED          = 0x0001; // 페이지 로딩완료
var MSG_SESSIONEXPIRED      = 0x0010; // 로그인이 필요한 페이지에서 로그인이 안된경우
var MSG_MOBILECERTNEED      = 0x0020; // 모바일인증(또는 모바일인증)이 필요한 페이지에서 모바일인증이 안된경우
var MSG_PUBLICCERTNEED      = 0x0030; // 공인인증필요한 페이지에서 공인인증이 안된경우
var MSG_ORDSAVESUCCESS      = 0x0500; // 각 상품신청 저장이 성공한 경우
var MSG_ORDSAVEFAIL         = 0x0550; // 각 상품신청 저장이 실패한 경우 이때에 Error_msg_cd , Error_msg_text 가 채워진다.
...
//------------------------------------------------------------------------------
// MOUSE DRAG DISABLED 2007-03-20 6:49오후 이재용
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// MOUSE RIGHT BUTTON CLICK DISABLED 2007-03-20 6:49오후 이재용
//------------------------------------------------------------------------------
// function right(e) {
