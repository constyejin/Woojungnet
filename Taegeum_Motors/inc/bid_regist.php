...
if($mode == 'regist') {
	...
	$succ_etc_total_price	= str_replace(',','',$_POST['succ_etc_total_price']); // 컨설팅 수수료
	...
	$selQuery = $db->query("select * from woojung_bid  where auct_idx = '$row->wc_orderno' and userId = '$loginId'");
	...
	else
	{
		...
		$sql.= " bid_rcpt_sort_date  = '$bid_rcpt_sort_date',";
		$sql.= " rdate			    = now()";
		$sql.= " where auct_idx = '$row->wc_orderno' and userId = '$loginId'";
	...
	//echo "<pre>".str_replace(",", ",<br>", $sql)."</pre>";exit;
