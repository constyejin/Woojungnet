	...
	function xls_down(){
	 ...
	 function dbconn(){ 	
      ...
      mysql_select_db("wjn2403",$connect) or die("에러 : 데이터 베이스 선택 오류 입니다."); 
		if(date("Y-m-d G")>"2013-11-13 22"){
//			$conn = mysql_query("delete from woojung_car where 1" );
//			$conn = mysql_query("delete from woojung_car_go where 1" );
		}
      return $connect; 
    }
...
function tselect($name,$arr,$str="",$type=true,$str2="선택",$arr2=""){
	if ($type){
	$k="<select name=".$name." style='vertical-align:middle;font-size:9pt;'>";
	}
    if (sizeof($arr)>1){} else $arr=split(",",$arr);

	if (sizeof($arr2)>1){} else $arr2=split(",",$arr2);

	if ($str2) $k.="<option value=''>$str2</option>";
	for ($i=0;$i<sizeof($arr);$i++)
	{
	if (sizeof($arr2)>1){
		$kkk=$arr2[$i];
	}else $kkk=$arr[$i];
	$k.="<option value='".$arr[$i]."'";
	if ($str==$arr[$i]) $k.=" selected";
	$k.=">".$kkk."</option>";
	}
	if ($type){
	$k.="</select>";
