...
$script = new scriptAlert(); 
	...
	if($mode == "delete"){
		if(count($chk) == 0 ){MsgMov("선택된 자료가 없습니다.","Sale_list.php");exit;}
		for($i=0;$i<count($chk);$i++){

			$sql="select * from woojung_part WHERE wc_idx = '$chk[$i]' ";
			...
			Query_string("DELETE FROM woojung_part WHERE wc_idx = '$chk[$i]'");
		}	
				
		if($back_url=="o"){
		MsgMov("선택된 정보가 삭제 되었습니다.","/sub08/sub08_1.php?page=$page&gubun4=$gubun4");
		}else{
