...
<?
	...
	if($logMode == 'login'){
		...
		$id = Row_string("SELECT * FROM $tb_name WHERE userId = '".$_SESSION["login_id_imsi"]."'");
		if( $rand_n=="9999"){
			$pw = Row_string("SELECT * FROM $tb_name WHERE userId='".$_SESSION["login_id_imsi"]."' ");
		}else{
			$pw = Row_string("SELECT * FROM $tb_name WHERE userId='".$_SESSION["login_id_imsi"]."' AND rand_num = '$rand_n'");
		}
		
		if($id[certify_error]==5){
			msg('고객센터로 문의하세요');			
			exit;
		}
		
		if(!$pw){			
			$query="update $tb_name set certify_error=certify_error+1 where userId='".$_SESSION["login_id_imsi"]."' ";
			mysql_query($query);
			$ce=$id[certify_error]+1;
			msg('인증번호를 '.$ce.'회 잘못입력 하셨습니다.');	
			exit;
		}else{
			$query="update $tb_name set certify_error=0 where userId='".$_SESSION["login_id_imsi"]."' ";
			mysql_query($query);
		}
		//print_r($pw);
...
//		msg($_SESSION[loginId]);
		...
		if($pw[pw_mod_date]<$p_date){
			echo '<script>parent.location.href="/login/newpass.php";</script>';
		}else{
