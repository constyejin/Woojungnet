...
if($idx){
	...
	$mod_car_trim=sql_fetch("select * from sale_out_trim where car_idx='$idx' order by trim_list asc ");
	$mod_car_trim_bt=sql_list("select * from sale_out_trim where car_idx='$idx' order by trim_list asc ");
	...
	$sale_out=sql_list("select * from sale_out where car_state='1' and car_name='$mod_car[car_name]' and idx!='$idx' order by idx desc ");
}
?>
<script>
...
<input type="hidden" name="car_color1" value="<?=$mod_color1[0][idx]?>">
<input type="hidden" name="car_color2" value="<?=$mod_color2[0][idx]?>">
    ...
    <div class="container-border">
      <div class="container">
        <section class="description">
          ...
          <p class="h3">
            해당차량관련 출고현황
            <a href="sub01.php" class="btn-more">+ 더보기</a>
          </p>
          <!-- 차량출고현황 리스트 -->
...
?>
              <li>
                <a href="sub01_view.php?idx=<?=$sale_out[$i][idx]?>&trim_idx=<?=$list_car_trim[idx]?>">
                  <div class="img-wrap">
				  <? if($car_img[0]){ ?>
            ...
            <div class="option interior-color">
              ...
              <div class="color-chip-list">
                <ul>
					  ...
					  ?>
                      <li <?=in_array($car_color2[$i][idx],$car_check2)?'class="selected"':""?>>
                        <a href="">
...
?>
		...
		<button type="button" id="btn_ch<?=$mod_car_trim_one[idx]?>" class="btn btn-round btn-outline-dark btn-sm on" ><?=$mod_car_trim_one[basic_price]?></button>
...
<div class="floating-bar">
  <div class="container">
    <span><?=$mod_car_name[basic_name]?> ㅣ </span>
    <span><?=$mod_car_trim_name[basic_price]?> ㅣ </span>
    <span class="price-label">총차량가격</span>
    <span class="price" id="float_write_3"><?=number($mod_car_trim_new[trim_price])?></span>
