...
if($car_type1) $mod_car_cate=sql_list("select * from category where cate_type1='$car_type1' order by cate_list asc ");
if($car_type2){
...
if($car_name){
	$mod_car=sql_fetch("select * from sale_car where car_name='$car_name' and car_type1='$car_type1' and car_type2='$car_type2' ");
	...
	if($select_trim){
		$trim_name=sql_fetch("select * from option_basic where idx='$select_trim' ");
		$car_trim_name=sql_fetch("select * from sale_car_trim where idx='$mod_car[idx]' and trim_idx='$select_trim' ");
	}
}
...
function ch_trim_new(){
	f=document.wform;
	f.target="";
	f.action="sub03_write.php";
	f.submit();
}
</script>
<?
	for($i=0;$i<count($mod_car_trim_bt);$i++){ 
	  echo "<script>";
	  echo "trim_name_save(".$mod_car_trim_bt[$i][idx].",'".$mod_car_trim_bt[$i][basic_price]."');";
	  echo "</script>";
	} 
?>
                ...
                <tr>
                  ...
                  <td colspan="3">
                    <div class="row">
                      ...
                      <div class="col-6">
                        <select class="form-select" aria-label="select" name="car_type2" onchange="ch_trim_new()">
...
<? } ?>
                ...
                <tr>
                  <th>차량명</th>
                  <td>
                    <select class="form-select car-name-trim" name="car_name" onchange="ch_trim_new()">
...
<? } ?>
                ...
                <tr>
                  <th>트림</th>
                  <td>
                    <select class="form-select car-name-trim" name="select_trim" onchange="ch_trim_new();">
                      <option value="">=== 트림선택 ===</option>
<?
	for($i=0;$i<count($mod_car_trim_bt);$i++){ 
?>
...
<? } ?>
                ...
                <tr>
                  ...
                  <td>
                    <div class="row align-items-center">
                      <div class="col-6">
                        <input type="text" class="form-control" name="car_price" value="<?=number($car_trim_name[trim_price])?>" id="number" onfocusout="wr_price()">
                      </div>
                      <div class="col-auto">
                        <span class="unit">
                          원
                ...
                <tr>
                  <th>차량설명</th>
                  <td colspan="3">
                    <textarea name="car_add" id="" cols="30" rows="8" class="form-control"><?=nl2br($car_trim_name[trim_explain])?></textarea>
                  </td>
                </tr>
                <tr>
                  <th>카탈로그<br>PDF</th>
...
<? for($i=0;$i<count($option_color1);$i++){ ?>
                      <li class="<?=in_array($option_color1[$i][idx],$car_check1)?"selected":""?>" id="outcolor1<?=$i?>">
                        ...
                        <div>
                          ...
                          <input type="text" class="form-control order-number" name="car_color3[]"/>
                        </div>
                      </li>
<? } ?>
                    </ul>
...
<? for($i=0;$i<count($option_color1_1);$i++){ ?>
                      <li class="<?=in_array($option_color1_1[$i][idx],$car_check1)?"selected":""?>" id="outcolor2<?=$i?>">
                        <a href="">
                        ...
                        <input type="checkbox" class="form-check-input" name="car_color1_1[]" value="<?=$option_color1_1[$i][idx]?>" <?=in_array($option_color1_1[$i][idx],$car_color1)?"checked":""?> onclick="c_color2(this,<?=$i?>,'outcolor2<?=$i?>')"><input type="checkbox" class="form-check-input" name="car_check2[]" value="<?=$option_color1_1[$i][idx]?>" <?=in_array($option_color1_1[$i][idx],$car_check1)?"checked":""?> style="display:none;">
						  <input type="hidden" name="car_color_1[]" value="<?=$option_color1[$i][idx]?>">
                          <input type="text" class="form-control order-number" name="car_color3_1[]"/>
                      </li>
<? } ?>
                    </ul>
                  </div>
...
<? for($i=0;$i<count($option_color2_1);$i++){ ?>
                      <li class="<?=in_array($option_color2_1[$i][idx],$car_check2)?"selected":""?>" id="outcolor4<?=$i?>">
                        <input type="checkbox" class="form-check-input" name="car_color2_1[]" value="<?=$option_color2_1[$i][idx]?>" <?=in_array($option_color2_1[$i][idx],$car_color2)?"checked":""?> onclick="c_color4(this,<?=$i?>,'outcolor4<?=$i?>')"><input type="checkbox" class="form-check-input" name="car_check4[]" value="<?=$option_color2_1[$i][idx]?>" <?=in_array($option_color2_1[$i][idx],$car_check2)?"checked":""?> style="display:none;">
                        <a href="">
...
<? for($i=0;$i<count($option_choice2);$i++){ ?>
                        <li class="<?=in_array($option_choice2[$i][idx],$car_check3)?"selected":""?>" id="oc2<?=$i?>">
                          <div class="check-wrap">
                            <input type="checkbox" class="form-check-input" name="car_choice2[]" value="<?=$option_choice2[$i][idx]?>" <?=in_array($option_choice2[$i][idx],$car_choice1)?"checked":""?> onclick="c_ch10(<?=$i?>,0,<?=$option_choice1[$i][ch_price]?>,'oc2<?=$i?>');"><input type="checkbox" class="form-check-input" name="car_check6[]" value="<?=$option_choice2[$i][idx]?>" <?=in_array($option_choice2[$i][idx],$car_check3)?"checked":""?> style="display:none;">
                            <input type="text" name="car_list1[]" class="form-control order-number">
                          </div>
                          <div class="item-wrap" onclick="c_ch6(<?=$i?>,0,<?=$option_choice2[$i][ch_price]?>);">
                            ...
                            <div class="title"><?=$option_choice2[$i][ch_name]?>
                              <a href="" class="tooltip toggle-layer-pop">
                                <span class="icon-tooptip" onclick="document.getElementById('pop_title').innerHTML='<?=$option_choice2[$i][ch_name]?>';document.getElementById('pop_img').src='/images/opt/<?=$option_choice2[$i][ch_file]?>';document.getElementById('pop_explain').innerHTML='<?=$option_choice2[$i][ch_explain]?>';document.getElementById('pop_price').innerHTML='<?=number($option_choice2[$i][ch_price])?>원';">!</span>
                              </a>
                            </div>
                            <div class="price">
...
<? for($i=0;$i<count($option_choice3);$i++){ ?>
                        <li class="<?=in_array($option_choice3[$i][idx],$car_check4)?"selected":""?>" id="oc3<?=$i?>">
                          <div class="check-wrap">
                            <input type="checkbox" class="form-check-input" name="car_choice3[]" value="<?=$option_choice3[$i][idx]?>" <?=in_array($option_choice3[$i][idx],$car_choice2)?"checked":""?> onclick="c_ch11(<?=$i?>,0,<?=$option_choice3[$i][ch_price]?>,'oc<?=$i?>');"><input type="checkbox" class="form-check-input" name="car_check7[]" value="<?=$option_choice3[$i][idx]?>" <?=in_array($option_choice3[$i][idx],$car_check4)?"checked":""?> style="display:none;">
...
<? for($i=0;$i<count($option_choice4);$i++){ ?>
                        <li class="<?=in_array($option_choice4[$i][idx],$car_check4)?"selected":""?>" id="oc4<?=$i?>">
                          <div class="check-wrap">
                            <input type="checkbox" class="form-check-input" name="car_choice4[]" value="<?=$option_choice4[$i][idx]?>" <?=in_array($option_choice4[$i][idx],$car_choice2)?"checked":""?> onclick="c_ch12(<?=$i?>,0,<?=$option_choice4[$i][ch_price]?>,'oc<?=$i?>');"><input type="checkbox" class="form-check-input" name="car_check8[]" value="<?=$option_choice4[$i][idx]?>" <?=in_array($option_choice4[$i][idx],$car_check4)?"checked":""?> style="display:none;">
                            <input type="text" name="car_list3[]" class="form-control order-number">
                          </div>
                          <div class="item-wrap" onclick="c_ch8(<?=$i?>,0,<?=$option_choice4[$i][ch_price]?>);">
                            ...
                            <div class="title"><?=$option_choice4[$i][ch_name]?>
                              <a href="" class="tooltip toggle-layer-pop">
                                <span class="icon-tooptip" onclick="document.getElementById('pop_title').innerHTML='<?=$option_choice4[$i][ch_name]?>';document.getElementById('pop_img').src='/images/opt/<?=$option_choice4[$i][ch_file]?>';document.getElementById('pop_explain').innerHTML='<?=$option_choice4[$i][ch_explain]?>';document.getElementById('pop_price').innerHTML='<?=number($option_choice4[$i][ch_price])?>원';">!</span>
                              </a>
                            </div>
                            <div class="price">
...
<? } ?>
          ...
          <div class="floating-bar on">
            <div class="container">
              <span id="float_write_1"><?=$option_basic[basic_name]?></span> ㅣ <span id="float_write_2"><?=$trim_name[basic_price]?></span> ㅣ <span>총차량가격</span> <span class="price" id="float_write_3">0</span>원
            </div>
          </div>
        </div>
        <!-- //기본정보 테이블 -->
...
<?
if($car_type1){
	if($car_type1=="화물차"){	echo '<script>document.getElementById("choice1").style.display="block";document.getElementById("choice2").style.display="none";document.getElementById("choice3").style.display="block";document.getElementById("choice4").style.display="none";document.getElementById("color1").style.display="block";document.getElementById("color1_1").style.display="none";document.getElementById("color2").style.display="block";document.getElementById("color2_1").style.display="none";</script>';
	}else if($car_type1=="캠핑카"){	echo '<script>document.getElementById("choice2").style.display="block";document.getElementById("choice1").style.display="none";document.getElementById("choice4").style.display="block";document.getElementById("choice3").style.display="none";document.getElementById("color1_1").style.display="block";document.getElementById("color1").style.display="none";document.getElementById("color2_1").style.display="block";document.getElementById("color2").style.display="none";</script>';
	}
