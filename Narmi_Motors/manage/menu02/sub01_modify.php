...
<? } ?>
                ...
                <tr>
                  <th>트림</th>
                  <td>
                    <select class="form-select car-name-trim" name="select_trim" onchange="location.href='sub01_modify.php?idx=<?=$idx?>&trim_idx='+this.value;">
                      <option>=== 트림선택 ===</option>
<?
	for($i=0;$i<count($mod_car_trim_bt);$i++){ 
		$trim_name=sql_fetch("select * from option_basic where idx='".$mod_car_trim_bt[$i][trim_idx]."' ");
?>
                          <option value="<?=$mod_car_trim_bt[$i][idx]?>" <?=$trim_idx==$mod_car_trim_bt[$i][idx]?"selected":""?>><?=$trim_name[basic_price]?></option>
<? } ?>
                    </select>
                  </td>
                </tr>
...
?>
		...
		<button type="button" id="btn_ch<?=$mod_car_trim_one[idx]?>" class="btn btn-round btn-outline-dark btn-sm<?=$on_bt?>" onclick="location.href='sub01_modify.php?idx=<?=$idx?>&trim_idx=<?=$mod_car_trim_bt[$i][idx]?>'"><?=$mod_car_trim_one[basic_price]?></button>
