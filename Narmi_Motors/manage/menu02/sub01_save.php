...
if(!$idx){
	...
	$mod_car=sql_fetch("select * from sale_car where car_type1='$car_type1' and car_type2='$car_type2' and  car_name='$car_name' ");
	if(!$mod_car[idx]){
...
//		$trim_price=$total_c3+$total_c4;
	...
	$mod_trim=sql_fetch("select * from sale_car_trim where car_idx='$mod_car[idx]' and trim_idx='".$select_trim."' ");
	$option_trim_list=sql_fetch("select * from sale_car_trim where idx='$select_trim'  ");
...
//	alert_p("등록완료","sub01_write.php?car_type1=$car_type1&car_type2=$car_type2&car_name=$car_name");
	alert_p("등록완료","sub01_view.php?idx=$mod_car[idx]&trim_idx=$mod_trim[idx]");
}else{
	$mod_car=sql_fetch("select * from sale_car where idx='$idx' ");
	if($mod_car[car_img]) $mod_car_img=explode("|:|" , $mod_car[car_img]);
