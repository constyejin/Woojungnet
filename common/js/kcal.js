...
//-----------------------------------------------CSS 적용을 위한 함수
	...
	function datePicker(tYear,tMonth,tDay,tYoil){ // 텍스트박스에 날짜 넣기 위해 만든 함수
		...
		calendar(tYear,tMonth,tDay);   //선택된걸로 다시 뿌려주장

		selDate.value=picDate.getFullYear()+"년 "+(picDate.getMonth()+1)+"월 "+picDate.getDate()+"일"+"("+tYoil+")";
		
		document.getElementById('rdate2').innerHTML = picDate.getFullYear()+"년 "+(picDate.getMonth()+1)+"월 "+picDate.getDate()+"일 "+tYoil+"요일"; //날짜및 시간에 
		document.getElementById('rdate_c').value = picDate.getFullYear()+"-"+(picDate.getMonth()+1)+"-"+picDate.getDate();//hidden값에;

	}


function calendar(tYear,tMonth,sday){ //달력 함수
	...
	if (tMonth==null)   //null 일경우, 처음 페이지가 로드 될때의 월은
		...
		for ( i=1; i<=lastDay; i++){       // 해당 월의 달력 
			//if(eDate.getFullYear()==nYear&&eDate.getMonth()==nMonth&&i==nDate && !sday){//오늘이고 선택된 날짜 업을때  today 스타일로 표시
