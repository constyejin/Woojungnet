...
$sale_car=sql_list("select * from sale_car where car_state='1' $where order by car_list desc limit $page_start, $page_row ");
$total_count=sql_total("select count(*) as cnt from sale_car where car_state='1' $where ");
...
<form name="cform" method="post" enctype="multipart/form-data" action="sub01_change.php" target="HiddenFrm">
...
<form name="sform" method="get" enctype="multipart/form-data">
        <div class="table-topper select-filter-group" style="text-align:right;">
          ...
          <div class="prefix">
          <a href="./sub01_write.php" class="btn btn-outline-secondary btn-sm">등록하기</a>
...
<input type="hidden" name="db_name" value="sale_car">
...
?>
            <li>
              <div class="check-wrap">
                <div class="prefix">
                  <input type="checkbox" class="form-check-input" value="<?=$sale_car[$i][idx]?>" name="checkidx[]">
                  <input type="number" name="car_list[]" value="<?=$sale_car[$i][car_list]?>" style="width:50px;" class="form-control">
				  <input type="hidden" name="car_idx[]" value="<?=$sale_car[$i][idx]?>" style="width:50px;" class="form-control">
				  <button type="button" onclick="list_ch(<?=$i?>);">변경</button>
                </div>
                <div class="suffix">
				<? if($sale_car[$i][car_view]=="Y"){ ?>
              ...
              <div class="vehicle-name">
                <a href="./sub01_view.php?idx=<?=$sale_car[$i][idx]?>&trim_idx=<?=$mod_car_trim[idx]?>">
                  <span class="category">
                    <?=$sale_car[$i][car_type1]?> > <?=$sale_car[$i][car_type2]?>
                  </span>
                  <span class="name">
