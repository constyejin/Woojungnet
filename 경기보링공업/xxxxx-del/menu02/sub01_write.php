...
if($car_name){
	...
	if($mod_car[idx]){
		$mod_car_trim_se=sql_list("select * from sale_car_trim where car_idx='$mod_car[idx]' ");
		if(count($mod_car_trim_se)>0){
			for($se=0;$se<count($mod_car_trim_se);$se++){
				$new_se[]=$mod_car_trim_se[$se][trim_idx];
			}
			$new_se2=implode(",",$new_se);
			$mod_car_trim_bt=sql_list("select * from option_basic where del='N' and basic_name='".$option_basic[basic_name]."' and basic_price!='' and idx not in (".$new_se2.") order by basic_list asc ");
		}else{
                ...
                <tr>
                  ...
                  <td colspan="3">
                    <div class="row">
                      ...
                      <div class="col-6">
                        <select class="form-select" aria-label="select" name="car_type2" onchange="ch_type2(this.value,'1');">
...
<? } ?>
                ...
                <tr>
                  <th>차량명</th>
                  <td>
                    <select class="form-select car-name-trim" name="car_name" onchange="ch_trim_new(this.value);">
...
<? } ?>
                ...
                <tr>
                  ...
                  <td>
                    <div class="row align-items-center">
                      <div class="col-6">
                        <input type="text" class="form-control" name="car_price" value="" id="number" onfocusout="wr_price()">
                      </div>
                      <div class="col-auto">
                        <span class="unit">
                          원
                ...
                <tr>
                  <th>차량설명</th>
                  <td colspan="3">
                    <textarea name="car_add" id="" cols="30" rows="8" class="form-control"></textarea>
                  </td>
                </tr>
                <tr>
                  <th>카탈로그<br>PDF</th>
...
<? for($i=0;$i<count($option_color1_1);$i++){ ?>
                      <li class="<?=in_array($option_color1_1[$i][idx],$car_check1)?"selected":""?>" id="outcolor1<?=$i?>">
                        <a href="">
                          <div class="color-chip" style="background-image: url(/images/opt/<?=$option_color1_1[$i][color_file]?>);" onclick="c_ch(<?=$i?>,this);"></div>
                        </a>
                        <span class="color-name"><?=$option_color1_1[$i][color_name]?></span>
                        <input type="checkbox" class="form-check-input" name="car_color1[]" value="<?=$option_color1_1[$i][idx]?>" <?=in_array($option_color1_1[$i][idx],$car_color1)?"checked":""?> onclick="c_color1(this,<?=$i?>,'outcolor1<?=$i?>')"><input type="checkbox" class="form-check-input" name="car_check1[]" value="<?=$option_color1_1[$i][idx]?>" <?=in_array($option_color1_1[$i][idx],$car_check1)?"checked":""?> style="display:none;">
						  <input type="hidden" name="car_color[]" value="<?=$option_color1[$i][idx]?>">
                          <input type="text" class="form-control order-number" name="car_color3[]"/>
                      </li>
<? } ?>
...
<? for($i=0;$i<count($option_color2_1);$i++){ ?>
                      <li class="<?=in_array($option_color2_1[$i][idx],$car_check2)?"selected":""?>" id="outcolor3<?=$i?>">
                        <input type="checkbox" class="form-check-input" name="car_color2[]" value="<?=$option_color2_1[$i][idx]?>" <?=in_array($option_color2_1[$i][idx],$car_color2)?"checked":""?> onclick="c_color3(this,<?=$i?>,'outcolor3<?=$i?>')"><input type="checkbox" class="form-check-input" name="car_check3[]" value="<?=$option_color2_1[$i][idx]?>" <?=in_array($option_color2_1[$i][idx],$car_check2)?"checked":""?> style="display:none;">
                        <a href="">
                          <div class="color-chip" style="background-image: url(/images/opt/<?=$option_color2_1[$i][color_file]?>);" onclick="c_ch3(<?=$i?>);"></div>
                        </a>
...
<? for($i=0;$i<count($option_choice2);$i++){ ?>
                        <li class="<?=in_array($option_choice2[$i][idx],$car_check3)?"selected":""?>" id="oc<?=$i?>">
                          <div class="check-wrap">
                            <input type="checkbox" class="form-check-input" name="car_choice1[]" value="<?=$option_choice2[$i][idx]?>" <?=in_array($option_choice2[$i][idx],$car_choice1)?"checked":""?> onclick="c_ch9(<?=$i?>,0,<?=$option_choice2[$i][ch_price]?>,'oc<?=$i?>');"><input type="checkbox" class="form-check-input" name="car_check5[]" value="<?=$option_choice2[$i][idx]?>" <?=in_array($option_choice2[$i][idx],$car_check3)?"checked":""?> style="display:none;">
                            <input type="text" name="car_list1[]" class="form-control order-number">
                          </div>
                          <div class="item-wrap" onclick="c_ch5(<?=$i?>,0,<?=$option_choice2[$i][ch_price]?>);">
                            <div class="img-wrap">
...
<? for($i=0;$i<count($option_choice4);$i++){ ?>
                        <li class="<?=in_array($option_choice4[$i][idx],$car_check4)?"selected":""?>" id="oc3<?=$i?>">
                          <div class="check-wrap">
                            <input type="checkbox" class="form-check-input" name="car_choice3[]" value="<?=$option_choice4[$i][idx]?>" <?=in_array($option_choice4[$i][idx],$car_choice2)?"checked":""?> onclick="c_ch11(<?=$i?>,0,<?=$option_choice4[$i][ch_price]?>,'oc3<?=$i?>');"><input type="checkbox" class="form-check-input" name="car_check7[]" value="<?=$option_choice4[$i][idx]?>" <?=in_array($option_choice4[$i][idx],$car_check4)?"checked":""?> style="display:none;">
                            <input type="text" name="car_list3[]" class="form-control order-number">
                          </div>
                          <div class="item-wrap" onclick="c_ch7(<?=$i?>,0,<?=$option_choice4[$i][ch_price]?>);">
                            <div class="img-wrap">
...
<? } ?>
          ...
          <div class="floating-bar on">
            <div class="container">
              <span id="float_write_1"><?=$option_basic[basic_name]?></span> ㅣ <span id="float_write_2"></span> ㅣ <span>총차량가격</span> <span class="price" id="float_write_3">0</span>원
            </div>
          </div>
        </div>
        <!-- //기본정보 테이블 -->
